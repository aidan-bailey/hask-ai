...
filterList :: [Valuation] -> [Valuation] -> [Valuation]
filterList _        [] = []
filterList []       _  = []
filterList (l : se) re = if search l re
  then l : filterList se re
  else filterList se re
  where search = \n list -> or [ n == l | l <- list ]

-- | int2bool function converts an integer to the corresponding binary (bool) value
int2bool :: Int -> [Bool]
int2bool i | i == 0    = [False]
...
find :: String -> Valuation -> Bool
find i s | null s    = False
         | i == c    = b
...
isSubset models1 models2 =
  and [ and [ interpSubset m1 m2 | m2 <- models2 ] | m1 <- models1 ]
